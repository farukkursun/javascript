...
ulkeler.forEach((a) => {
  a.onclick = async() => {
    await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${a.id}`)
      .then((response) => response.json())
      .then((results) => printScreen(results.meals));
  };
});
// myInput.onInput = (harf) =>{
//    myList.filter(harf => harf.includes(harf))
// }
// const x = dizi.filter((a)=> a.isim.includes(harf.target.value)
